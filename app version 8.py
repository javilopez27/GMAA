...
class VentanaPrincipalGMAA(QMainWindow):
    ...
    def open_NodeInfo(self):
        ...
        if not sender.unitType:
            ...
            self.vNodeInfo.textMinRangeCont.setPlainText(sender.minRange)
            self.vNodeInfo.textMaxRangeCont.setPlainText(sender.maxRange)
        ...
        self.vNodeInfo.tableWidgetVAC.setRowCount(1)
        ...
        self.vNodeInfo.tableWidgetVAC.setHorizontalHeaderItem(0, QTableWidgetItem("Attribute"))
        self.vNodeInfo.tableWidgetVAC.setHorizontalHeaderItem(1, QTableWidgetItem(""))
        ...
        if not (index == None):
            ...
            for alternative in alternatives:
                ...
                self.vNodeInfo.tableWidgetVAC.setHorizontalHeaderItem(column+2, header_item)
                self.vNodeInfo.tableWidgetVAC.setHorizontalHeaderItem(column+3, QTableWidgetItem(""))
                ...
                if isinstance(elementCD, ContinuousAttribute):
                    ...
                    item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
                    self.vNodeInfo.tableWidgetVAC.setItem(0, column+2, item)
                    item2.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
                    self.vNodeInfo.tableWidgetVAC.setItem(0, column+3, item2)
                    ...
                    self.vNodeInfo.tableWidgetVAC.setItem(0, column+2, QTableWidgetItem(selected_option))
    ...
    def min_alt(self,sender):
        self.vNodeInfo.tableWidgetVAC.clear()
        attributeName = sender.text()
        textMinRange = sender.minRange
        textMaxRange = sender.maxRange
        index = None
        ...
        if not (index == None):
            ...
            for alternative in alternatives:
                ...
                if isinstance(elementCD, ContinuousAttribute):
                    ...
                    aver = str(float((value1+value2)/2))
                    item = QTableWidgetItem(aver)
                    item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
                    self.vNodeInfo.tableWidgetVAC.setItem(0, column+2, item)
                elif isinstance(elementCD, DiscreteAttribute):
                    self.vNodeInfo.tableWidgetVAC.setSpan(0, column+2, 1, 2) 
...
class VentanaAltCon(QMainWindow):
    ...
    def modify_Alt(self): # en uno vemos los atributos de esa alternativa y apretando un button vamos a cambiarlo
        ...
        self.vAddAlt.show()
        self.vAddAlt.ejecutar_funcion = True
            ...
            if isinstance(attribute, ContinuousAttribute) and self.ejecutar_funcion:
                try:
                    ...
                    if (float(attribute.value1) <= value1) and (float(attribute.value2) >= value2):
                        ...
                        QMessageBox.warning(self, 'Warning', 'Some parametres are not within the range set out in the attribute.')
                        print(f"float(attr.value1({float(attribute.value1)})) <= value1({value1})) and (float(attr.value2({float(attribute.value2)})) >= value2({value2})")
                        return
                except (ValueError, AttributeError):
                    QMessageBox.warning(self, 'Warning', 'Complete all values correctly.')
                    return
