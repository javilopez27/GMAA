import sys
from PyQt6.QtWidgets import QApplication,QGridLayout,QInputDialog,QGraphicsLineItem, QGraphicsView,QScrollArea, QGraphicsView, QFrame, QWidget,QToolBar, QSpacerItem,QMainWindow, QFileDialog, QLabel, QMenu, QPushButton, QVBoxLayout, QStatusBar, QHBoxLayout, QLineEdit, QMessageBox
...
class VentanaPrincipalGMAA(QMainWindow):
    ...
    #         if label1.connections:
    #             for j in range(len(label1.connections)):
    #                 connection = label1.connections[j]
    #                 line = DLineFrame()
    #                 self.layout.addWidget(line,connection.cRow,connection.cColumn-1,connection.cRow+1,2)
    #                 line.lower()
...
class VentanaLogIn(QMainWindow):
    ...
    def checkUserPw(self):
        if self.lineUser.text() == '1' and self.linePw.text() == '1':
            ...
            main_window_gmaa.showFullScreen()
...
class LabelNode(QLabel):
    ...
    def create_lines_layout(self):
        ...
        if labelPO is not None and labelPO.connections:
            connection_listPO = list(labelPO.connections)
            for k in range(len(connection_listPO)):
                if k == 0:
                    ...
                    line = DLineFrame()
                    parent.parent.layout.addWidget(line,labelPO.cRow,connection.cColumn-1,numero-1,2)
                    line.lower()
                    parent.parent.lines.append(line)
        ...
        for i in range(1,len(parent.parent.labels)):
            ...
            if label.connections:
                for j in range(len(label.connections)):
                    ...
                    connection_list = list(label.connections)
