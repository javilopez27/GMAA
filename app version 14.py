...
class VentanaPrincipalGMAA(QMainWindow):
    ...
    def open_NodeInfo(self):
        ...
        self.vNodeInfo.AttNameChart.setText(sender.text())
        ...
        for i, attr in enumerate(attributes):
            if attr.unitType == "Continuous":
                print("P: attr.minRange: "+str(float(attr.minRange))+", attr.minRange: "+str(float(attr.maxRange)))
                if (str(float(attr.minRange)) == str(textMinRange)) and (str(float(attr.maxRange)) == str(textMaxRange) and (attributeName == attr.text())):
                    index = i
                    break
                else:
                    index = None
        ...
        widget = QWidget()
        # window.setCentralWidget(widget)
        layoutVCU = self.vNodeInfo.hLayoutVCU
        layoutVCU.addWidget(widget)
        ...
        chart_view = ChartView(text_edit=self.vNodeInfo.plainTextAVC)
        chart_view.setChart(chart)
        chart_view.setFrameShape(QFrame.Shape.NoFrame)
        chart_view.setRenderHint(QPainter.RenderHint.Antialiasing)
        chart_view.setBackgroundBrush(brush)

        layoutVCU.addWidget(chart_view)
        ...
        def enable_apply():
            if sender.finalNode:
                ...
                if (self.vNodeInfo.textName.toPlainText().strip() != "" and self.vNodeInfo.textDescription.toPlainText().strip() != "" and self.vNodeInfo.textLabel.toPlainText().strip() != ""):
                    self.vNodeInfo.buttonApply.setEnabled(True)
    ...
    def modifyAltCon(self,sender):
        ...
        for i, attr in enumerate(attributes):
            if attr.unitType == "Continuous":
                ...
                if (str(float(attr.minRange)) == str(textMinRange)) and (str(float(attr.maxRange)) == str(textMaxRange) and (sender.text() and attr.text())):
                    index = i
                    break
                else:
                    index = None
        ...
        if sender.unitType == "Continuous":
            ...
            for i,altern in enumerate(alternatives):
                elementCD = altern.attr[index]
                print(f"P: elementCD.name: {elementCD.name}, self.vModAlt.textAttributeName.toPlainText()  {self.vModAlt.textAttributeName.toPlainText()}\n")
...
class VentanaAltCon(QMainWindow):
    ...
    def __init__(self,parent=None):
        ...
        for i,altern in enumerate(alternatives):
            ...
            item3.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            self.tableWidgetAC.setItem(1, column+5, item3)
            column += 3
                        ...
                        for at in attributes:
                            ...
                            if at.text() == attr.name:
                                try:
                                    ...
                                    valueT1 = self.vAddAlt.tableWidgetAA.item(j, 0).text()
                                    valueT2 = self.vAddAlt.tableWidgetAA.item(j, 1).text()
                                    print(f"{value1} {valueT1} {valueT2} {value2}")
                                    if (float(valueT1) >= value1) and (float(valueT2) <= value2):
                                        continue
                                    else:
...
class VentanaCUV(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        if attributesnames:
            ...
            self.AttNameChart.setText(attributesnames[0])
            
        else:
            self.AttNameChart.setText("")
        ...
        self.tableAltName.verticalHeader().setVisible(False)  # Ocultar el encabezado vertical
        self.tableAltName.horizontalHeader().setVisible(False)  # Ocultar el encabezado horizontal
        self.tableAltName.setEditTriggers(QTableWidget.EditTrigger.NoEditTriggers)  # Deshabilitar la edición de los elementos
        ...
        chart_view = ChartView(text_edit=self.plainTextAVC)
        chart_view.setChart(chart)
        chart_view.setFrameShape(QFrame.Shape.NoFrame)
        chart_view.setRenderHint(QPainter.RenderHint.Antialiasing)
        chart_view.setBackgroundBrush(brush)
    ...
    def on_combo_box_changed(self,text):
        ...
        x_axis.setLabelFormat("%.1f")
        x_axis.setTickCount(2)  # Número de etiquetas en el eje X
        x_axis.setRange(float(selected_attr.minRange), float(selected_attr.maxRange))
    ...
    def on_item_double_clicked(self,item):
        if isinstance(item, QTableWidgetItem):
            ...
            self.AttNameChart.setText(value)
                ...
                if attr.name == self.textAttributeName.toPlainText():
                    if isinstance(attr, ContinuousAttribute):
                        ...
                        try:
                            ...
                            QMessageBox.critical(self.tableWidgetMAC, "Error", "It cannot convert the cell value")
                    elif isinstance(attr, DiscreteAttribute):
...
class ChartView(QChartView):
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.chart:
            ...
            self.text_edit.setPlainText(f"X: {x_value: .2f}\nY: {y_value: .2f}")
